...
public class EncryMojo extends AbstractMojo {
	...
	private void writePasswordToCurDir() {
		...
		try {
			FileUtils.write(
					...
					this.encryPassword);
			getLog().warn("密码已保存到当前目录下的[decry.psd]文件中,请注意保存");
		} catch (IOException e) {
	...
	private void resetMainClass() {
		...
		System.out.println("找 到 主 类 文 件 :\t" + mainClassFile.getAbsolutePath());
	...
	private void encryClassFile() {
		...
		for (EncryItem encryItem : this.encryItemes) {
			...
			getLog().info("待加密的资源路径:\t" + encryFile.getAbsolutePath() + "\t是否是文件:" + encryFile.isFile());
